...
@dataclass
class DataStore:
    utenti: Dict[str, Utente] = field(default_factory=dict)
    gruppi: Dict[str, Gruppo] = field(default_factory=dict)
    messaggi: Dict[str, Messaggio] = field(default_factory=dict)
    incontri: Dict[str, Incontro] = field(default_factory=dict)
    materiali: Dict[str, Materiale] = field(default_factory=dict)
    segnalazioni: Dict[str, Segnalazione] = field(default_factory=dict)
    notifiche: Dict[str, Notifica] = field(default_factory=dict)
...
def salva_datastore(datastore: DataStore, percorso: str = DEFAULT_DATA_PATH) -> None:
    os.makedirs(os.path.dirname(percorso), exist=True)
